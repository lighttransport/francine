...
FrancineWS.prototype.init = function init() {
  ...
  _this._ws.on('message', function onMessage(rawMessage) {
    ...
    if (message.command === 'getAuthorizeStatus') {
    ...
    if (message.command === 'registerResourceToken') {
      _this.render();
    }
    ...
    if (message.command === 'createExecution') {
