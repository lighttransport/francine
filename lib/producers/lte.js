...
LteProducer.prototype._sendUpdate = function _sendUpdate(sessionName, update) {
  ...
  return function() {
    var d = Q.defer();
    _this.worker.log('LteProducer', 'applying update:' + JSON.stringify(update));
...
LteProducer.prototype.produce = function produce(task) {
  ...
  p = p.then(function() {
    ...
    _this.worker.log('LteProducer', 'applied updates:' + _this.processes[sessionName].appliedUpdates);
    _this.worker.log('LteProducer', 'available updates:' + task.session.updates.length);
