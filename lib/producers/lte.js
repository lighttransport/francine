...
LteProducer.prototype._spawn = function _spawn(task) {
  ...
  return function() {
    ...
    spawned.stdout.on('data', function(data) {
      _this.worker.log('LteProducer', 'Stdout: ' + data.toString('utf-8'));
      if (data.toString('utf-8').indexOf('wait an connection') >= 0) {
        d.resolve();
      }
    });

    spawned.stderr.on('data', function(data) {
      _this.worker.log('LteProducer', 'Stderr: ' + data.toString('utf-8'));
    });
...
LteProducer.prototype._sendUpdate = function _sendUpdate(sessionName, update) {
  ...
  return function() {
    ...
    _this.worker.log(
      'LteProducer',
      'applying update:' + JSON.stringify(update));
...
LteProducer.prototype.produce = function produce(task) {
  ...
  p = p.then(function() {
    ...
    _this.worker.log(
      'LteProducer',
      'applied updates:' + _this.processes[sessionName].appliedUpdates);
    _this.worker.log(
      'LteProducer',
      'available updates:' + task.session.updates.length);
    ...
    _this.processes[sessionName].appliedUpdates = task.session.updates.length;
