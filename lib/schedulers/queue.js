...
QueueScheduler.prototype.createExecution = function (options) {
    ...
    for (var i = 0; i < 4; i++) {
        producingTaskNames.push(self._createProducingTask(self.sessions[executionName], execution));
    }
    self.schedule();
    ...
    .then(function (producings) {
        ...
        var reducingTaskName = self._createReducingTask(self.sessions[executionName], execution);
        self.schedule();
