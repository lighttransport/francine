...
QueueScheduler.prototype.createExecution = function (options, done) {
    var self = this;
...
QueueScheduler.prototype.schedule = function () {
    ...
    var worker = self.instance.getWorkers()[workerName];
...
QueueScheduler.prototype._updateWorkers = function () {
    var self = this;

    if (self.instance.getTimestamp() === self.instancesTimestamp) {
        return;
    }

    self.instancesTimestamp = self.instance.getTimestamp();

    // TODO(peryaudo): It is only valid when using 'static' instance manager.
    self.unusedWorkers = Object.keys(self.instance.getWorkers());
};

QueueScheduler.prototype._receiveAndReturn = function (worker, taskName, execution, done) {
    var self = this;
...
QueueScheduler.prototype.dispatchFinish = function (info) {
    ...
    var worker = self.instance.getWorkers()[info.workerName];
    ...
    switch (info.task.type) {
        ...
        case 'REDUCING':
            var execution = self.executions[executionName];
            var done = self.executions[executionName].done;
            delete self.executions[executionName];
            self._receiveAndReturn(worker, info.task.name, execution, done);
