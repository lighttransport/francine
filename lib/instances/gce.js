...
var GceInstance = function (master) {
    ...
    // self.masterMachineType = 'f1-micro';
    // self.workerMachineType = 'f1-micro';
};
...
GceInstance.prototype.resize = function (size) {
    var self = this;
    var d = Q.defer();
    if (self.size === size) {
        self.master.log('GceInstance', 'The cluster already has size of ' + size);
        d.resolve();
        return;
    }
    self.master.log('GceInstance', 'Resizing GCE managed instance group...');
    self._retrieveAccessToken(function (headers) {
        request({
            method: 'POST',
            uri: 'https://www.googleapis.com/replicapool/v1beta2/projects/' + self.project + '/zones/' + self.zone + '/instanceGroupManagers/francine-worker-group/resize',
            headers: headers,
            json: true,
            body: { size: size }
        }, function (error, response, body) {
            ...
            self.master.log('GceInstance', 'Resized GCE managed instance group: ' + JSON.stringify(body));
            d.resolve();
        });
    });
    d.resolve();
    ...
    .then(function () {
        ...
        return self._executeCommands([
            ...
            ['creating worker instance template', 0,
             'gcloud', ['compute', 'instance-templates', 'create', 'francine-worker-template',
                        '--quiet',
                        '--project',       self.project,
                        '--machine-type',  self.workerMachineType,
                        '--scopes',        'compute-rw',
    ...
    .then(function () {
        return self._executeCommands([
            ['deleting worker instance template', 0,
             'gcloud', ['compute', 'instance-templates', 'delete', 'francine-worker-template',
                        '--quiet',
                        '--project',       self.project]],
