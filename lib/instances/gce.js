...
function GceInstance(master, configs) {
  ...
  if (configs && configs.gce) {
    ...
    _this.preemptiveVMRate = configs.gce.preemptiveVMRate;
  }
  ...
  return _this._executeCommands([
    ...
    ['copying francine package', 0,
     'gcloud', ['compute', 'copy-files',
          _this.master.getPackageName(), 'francine-builder:~/',
          '--quiet',
          '--project', _this.project,
          '--zone',  _this.zone]],
...
GceInstance.prototype.setup = function setup(startMaster) {
  ...
  .then(function() {
    ...
    var instanceTemplateArg = ['compute', 'instance-templates',
    ...
    // @fixme { Current GCE API looks like it doesn't allow fractional allocation of preemptive VMs }
    if (_this.preemptiveVMRate > 0) {
      // Must set --maintainance-policy to TERMINATE, otherwise you'll get an error like this:
      //
      ...
      //
      instanceTemplateArg = instanceTemplateArg.concat(['--preemptible', '--maintenance-policy', 'TERMINATE'])
    }
