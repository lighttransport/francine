...
function GceInstance(master) {
  var _this = this;

  _this.master = master;

  _this.project = 'gcp-samples';
  _this.zone = 'us-central1-a';
  _this.masterMachineType = 'n1-standard-1';
  _this.workerMachineType = 'n1-standard-1';
  // _this.masterMachineType = 'f1-micro';
  // _this.workerMachineType = 'f1-micro';
}
...
GceInstance.prototype.resize = function resize(size) {
  ...
  _this.master.log('GceInstance', 'Resizing GCE managed instance group to ' + size + '...');
  ...
  .then(function(headers) {
    request({
      ...
      uri: 'https://www.googleapis.com/replicapool/v1beta2' +
        ...
        '/instanceGroupManagers/francine-worker-group/resize',
      headers: headers,
      form: { size: size }
    }, function(error, response, body) {
      if (error) {
...
GceInstance.prototype._executeCommand = function _executeCommand(command) {
  var _this = this;
...
GceInstance.prototype._executeCommands = function _executeCommands(commands) {
  var _this = this;
  ...
  commands.map(function(command) {
    p = p.then(function() {
      return _this._executeCommand(command);
    });
  });
  ...
  return _this._executeCommands([
    ...
    ['copying francine package', 0,
     'gcloud', ['compute', 'copy-files',
          _this.master.getPackageName(), 'francine-builder:/home/peryaudo/',
          '--quiet',
          '--project', _this.project,
          '--zone',  _this.zone]],
    ...
    ['executing setup script', 0,
     'gcloud', ['compute', 'ssh', 'francine-builder',
          '--command', 'sudo /home/peryaudo/scripts/setup.sh gce',
          '--quiet',
          '--project', _this.project,
          '--zone',  _this.zone]],
    ...
    return _this._executeCommands([
      ...
      ['creating worker instance template', 0,
       'gcloud', ['compute', 'instance-templates',
            'create', 'francine-worker-template',
            '--quiet',
            '--project',     _this.project,
            '--machine-type',  _this.workerMachineType,
            '--scopes',    'compute-rw',
      ...
      ['creating worker managed instance group', 0,
       'gcloud', ['preview', 'managed-instance-groups',
            '--zone',      _this.zone,
            'create', 'francine-worker-group',
            '--quiet',
            '--project',     _this.project,
            '--base-instance-name', 'francine-worker',
            '--size',      '1',
            '--template',    'francine-worker-template']],
  ...
  .then(function() {
    return _this._executeCommands([
      ['deleting worker instance template', 0,
       'gcloud', ['compute', 'instance-templates',
            'delete', 'francine-worker-template',
            '--quiet',
            '--project',     _this.project]],
