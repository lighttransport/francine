...
var PngReducer = function (worker) {
    var self = this;
    self.worker = worker;
};

PngReducer.prototype.reduce = function (task) {
    var self = this;

    var sources = task.execution.tasks;

    // self.worker.log('reduce', 'Reducing tasks of [' + sources.map(function (source) { return source.taskName; }).join(',') + ']');

    return Q.all(sources.map(_retrieve))
    .then(_reduce)
    .then(_save(task, self.worker));
};

module.exports = PngReducer;
