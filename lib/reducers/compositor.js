      ...
      [
        ...
        Array.prototype.concat.apply([],
          images.map(function(image) {
            return [Math.floor(image.weight).toString(), image.savedFileName];
          }))));
...
CompositorReducer.prototype.reduce = function reduce(task) {
  ...
  // _this.worker.log('CompositorReducer', 'Reducing start: ' + task.name);
  ...
  .then(function() {
    var d = Q.defer();
    d.resolve(weight);
    // _this.worker.log('CompositorReducer', 'Reducing end: ' + task.name);
    return d.promise;
  });
};
