...
Worker.prototype.getTemporaryDirectory = function getTemporaryDirectory() {
  ...
  return _this.temporaryDirectory;
};
...
Worker.prototype._initializeRpc = function _initializeRpc() {
  ...
  _this.rpcServer = new JsonRpcServer(new JsonRpcServerTcp(_this.port), {
    ...
    run: function run(task, callback) {
      ...
      switch (task.type) {
        case 'PRODUCING':
          ...
          .then(function() {
            startTime = Date.now();
            return _this.producers[task.session.options.producer].produce(task);
          })
          .then(function() {
          ...
          break;
        case 'REDUCING':
          p = _this.reducers[task.session.options.reducer].reduce(task);
          break;
      }

      p.then(function() {
        _this._sendFinishTask(task, Date.now() - startTime);
      }).done();
    ...
    deleteCache: function deleteCache(info, callback) {
      ...
      _this.producers[session.options.producer].deleteCache(info.sessionName);
...
Worker.prototype._initializeReducers = function _initializeReducers() {
  var _this = this;
  _this.reducers = {};
  _this.reducers.png = new PngReducer(_this);
};
...
Worker.prototype._sendFinishTask = function _sendFinishTask(task, elapsedTime) {
  var _this = this;
...
function _sendFinishFetching(task, elapsedTime) {
  ...
  var client = new JsonRpcClient(
      new JsonRpcClientTcp(_this.master.host, _this.master.port));
  client.register('finish');
  client.finish({
    type: 'FETCHING',
    workerName: _this.getName(),
  ...
  for (i = 0; i < files.length; i++) {
    ...
    switch (file.type) {
      ...
      case 'dropbox':
        ...
        resource.initializeInWorker(_this, task.tokens[file.type]);
        break;
    }
...
Worker.prototype._linkResources = function _linkResources(task) {
  ...
  for (var i = 0; i < files.length; i++) {
    fs.linkSync(
        ...
        _this.temporaryDirectory +
        '/executions/' + task.execution.name + '/' + files.dst);
  }
