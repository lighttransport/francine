...
function setupAndExecute(setupFn, executeFn, repeat, parallel) {
  return function() {
    ...
    .then(function(session) {
      ...
      for (var i = 0; i < repeat; i++) {
        p = p.then(executeFn(session, parallel));
...
function variate(setupFn, exp) {
  return function() {
    ...
    for (var i = 0; i < exp; i++) {
      p = p.then(_header(1 << i));
      p = p.then(setupAndExecute(setupFn, execute, 10, (1 << i)));
      p = p.then(_footer);
    }
