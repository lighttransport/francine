...
Master.prototype.createExecution = function createExecution(options, token) {
  ...
  return {
    execution: _this.getExecution(executionName),
    reduced: p
  };
};
...
function registerResourceTokenByToken(resourceName, code, authToken) {
  ...
  _this.generateResourceToken(code)
...
Master.prototype.getAuthorizeUrl = function getAuthorizeUrl(resourceName) {
  if (!this.resources[resourceName]) {
    return { error: 'invalid resource name' };
  }

  return { authorizeUrl: this.resources[resourceName].getAuthorizeUrl() };
};
