...
Master.prototype.getSession = function (sessionName) {
    var self = this;
    return self.sessions[sessionName];
};

Master.prototype.getExecution = function (executionName) {
    var self = this;
    return self.executions[executionName];
};
...
Master.prototype.logStatus = function () {
    var self = this;
    self.log('Master', 'Sessions: ' + Object.keys(self.sessions).length + ' Executions: ' + Object.keys(self.executions).length);
};
...
Master.prototype._dispatchFinish = function (info) {
    ...
    if (info.type === 'TASK') {
        if (info.task.type === 'PRODUCING') {
            self.executions[info.task.execution.name].time.producing += info.elapsedTime;
            self.executions[info.task.execution.name].progress++;
        } else if (info.task.type === 'REDUCING') {
            self.executions[info.task.execution.name].time.reducing += info.elapsedTime;
        }
        ...
        self.executions[info.executionName].time.fetching += info.elapsedTime;
        self._appendCachedWorker(info.workerName, info.cachedSessions);
...
Master.prototype.deleteSession = function (sessionName) {
    ...
    self.sessions[sessionName].executionNames.map(function (executionName) {
...
Master.prototype.createExecution = function (options, async) {
    var self = this;
    var d = Q.defer();
    ...
    var p = self.delayUntilFinishFetching(initialProducingTaskName)
    .then(function () {
    ...
    .then(function (reducing) {
        var d = Q.defer();
        self.executions[executionName].time.total = Date.now() - startTime;
        self.executions[executionName].last = reducing;
