...
var initializeWsApi = require('./apis/websocket');
...
Master.prototype.start = function start() {
  ...
  _this.loop(_this.manageInterval, manageInstances).done();
  // To speed up launch in local environment
  setTimeout(manageInstances, 2000);
  ...
  // Begin debugging assertion
  /*
  setInterval(function() {
    var executionNames = _this.scheduler.getQueuedExecutionNames();
    for (var executionName in _this.executions) {
      if (_this.executions.hasOwnProperty(executionName)) {
        var execution = _this.executions[executionName];
        if (!execution.finished && !executionNames[executionName]) {
          throw 'Fatal: ' + executionName + ' not in queue (bug in executor)';
        }
      }
    }
  }, 5 * 1000);
  */
  // End debugging assertion
...
Master.prototype.getStatus = function getStatus() {
  return {
    sessions: Object.keys(this.sessions).length,
    executions: Object.keys(this.executions).length,
    timeouts: Object.keys(this.taskTimeoutIDs).length
  };
};
...
Master.prototype._initializeWsApi = function _initializeWsApi() {
  this.ws = initializeWsApi(this);
};
...
Master.prototype.connectToWorker = function connectToWorker(worker) {
  if (worker.client) {
    return;
  }

  worker.client = new JsonRpcClient(
  ...
  worker.client.register('ping');
  worker.client.register('run');
  worker.client.register('deleteCache');
};
...
Master.prototype = concatObject(Master.prototype, tasks);
