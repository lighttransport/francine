...
Master.prototype.getId = function getId() {
  var _this = this;
  _this.seed++;
  return (Date.now() | 0) + '-' + _this.seed;
};

Master.prototype.getResourceToken = function getResourceToken(resourceType) {
  var _this = this;
  return _this.resources[resourceType].getToken();
};

Master.prototype.getSession = function getSession(sessionName) {
  var _this = this;
  return _this.sessions[sessionName];
};

Master.prototype.getExecution = function getExecution(executionName) {
  var _this = this;
  return _this.executions[executionName];
};
...
function getNextCachedWorker(sessionName) {
  ...
  if (_this.sessions[sessionName].cachedWorkers.length === 0) {
    return null;
  }
...
function _initializeInstanceManager() {
  ...
  switch (_this.instanceManagerType) {
    case 'static':
      _this.instanceManager = new StaticInstanceManager(_this, 8);
      break;
  }
...
Master.prototype._initializeRestApi = function _initializeRestApi() {
  var _this = this;
  _this.app = initializeRestApi(_this);
};

Master.prototype._initializeResources = function _initializeResources() {
  var _this = this;
  _this.resources = {};
  _this.resources.dropbox = new DropboxResource();
  _this.resources.dropbox.initializeInMaster(_this, _this.configs.dropbox);
  _this.resources.dropbox.registerEndpoint(_this.app);
};
...
Master.prototype.runTask = function runTask(workerName, task) {
  ...
  if (_this.taskTimeouts[task.name]) {
    ...
    _this.log('Master', 'Starting task timeout of ' + task.name + ' with ' + timeout);
    delete _this.taskTimeouts[task.name];
    _this.taskTimeoutIDs[task.name] = setTimeout(function() {
      _this.fireTaskTimeout(task.name);
    }, timeout);
  }
...
Master.prototype.setTaskTimeout = function setTaskTimeout(taskName, timeout) {
  ...
  if (_this.finishFetchingDefers[taskName] || _this.finishTaskDefers[taskName]) {
    _this.log('Master', 'Set task timeout of ' + taskName + ': ' + timeout + 'ms');
    _this.taskTimeouts[taskName] = timeout;
  }
};

Master.prototype.fireTaskTimeout = function fireTaskTimeout(taskName) {
  var _this = this;

  _this.log('Master', 'fireTaskTimeout called');

  var d;
  if (_this.finishFetchingDefers[taskName]) {
    d = _this.finishFetchingDefers[taskName];

    _this.log('Master', 'Fetching timeout of ' + taskName);

    d.reject('fetching timeout');
  }
  if (_this.finishTaskDefers[taskName]) {
    d = _this.finishTaskDefers[taskName];

    _this.log('Master', 'Task timeout of ' + taskName);

    d.reject('task timeout');
  }
};
...
Master.prototype.clearTaskTimeout = function clearTaskTimeout(taskName) {
  ...
  if (_this.taskTimeoutIDs[taskName]) {
    _this.log('Master', 'Clearing task timeout of ' + taskName);
...
Master.prototype.createSession = function createSession(options) {
  var _this = this;

  var sessionName = 'session' + _this.getId();

  _this.sessions[sessionName] = {
    name: sessionName,
...
Master.prototype.deleteSession = function deleteSession(sessionName) {
  var _this = this;
...
Master.prototype.createExecution = function createExecution(options, async) {
  var _this = this;
  var d = Q.defer();

  if (!_this.sessions.hasOwnProperty(options.sessionName)) {
    _this.log('Master', 'No such session available! ' + options.sessionName);
    d.reject();
    return d.promise;
  }
  ...
  _this.log('Master', 'Execution ' + executionName + ' created.');
  ...
  .then(function() {
    ...
    for (var i = 1; i < execution.parallel; i++) {
      producingTaskNames.push(
          _this.scheduler.createProducingTask(session, execution, i));
    }
    _this.scheduler.schedule();

    var producingTasks = producingTaskNames.map(function(producingTaskName) {
      return _this.delayUntilFinishTask(producingTaskName).then(function (info) {
  ...
  var producingFinished = function producingFinished(producing) {
    ...
    if (currentUnit === reducingUnit || currentProduced === totalProduced) {
      reducingPromises.push(
          _this.delayUntilFinishTask(
            _this.scheduler.createReducingTask(session, execution, produceds)));
      _this.scheduler.schedule();
      produceds = [];
      currentUnit = 0;
    }
